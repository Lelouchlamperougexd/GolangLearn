...
const Dashboard: FunctionComponent = () => {
  ...
  // Load overview, chats and applications on mount
  useEffect(() => {
    getDashboardOverview()
      .then(setOverview)
      ...
      <div className={styles.settingsContainer}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 400 }}>
          ...
          <div className={styles.formGroup}>
            ...
            <div style={{ position: "relative" }}>
              <input
                type={showConfirm ? "text" : "password"} className={styles.formInput}
                value={confirmPassword}
                onChange={e => { setConfirmPassword(e.target.value); setPasswordError(""); }}
                placeholder="Повторите новый пароль"
                style={{ paddingRight: 36, borderColor: passwordError && confirmPassword ? "#ff4d4f" : undefined }}
              />
              <button type="button" onClick={() => setShowConfirm(p => !p)} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#939393", display: "flex", padding: 0 }}>
                {showConfirm
